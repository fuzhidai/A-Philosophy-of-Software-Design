...
注释的目的是确保系统的结构和行为，对读者来说足够清晰，这样他们能够快速的找到所需的信息，并在满怀信心的状态下，完成系统的修改。有些信息只需通过某种方式在代码中展示，对读者来说就已足够清晰，但仍有许多其它的信息，无法轻易从代码中推断得出。注释则可以补充这些信息。

注释应该描述那些代码中并不明显的事物，遵循该规则时，“明显” 是从任何首次阅读你代码的读者（而非你自己）视角出发。编写注释时，尝试将自己放在读者的视角上，问问自己，什么是其他人需要知道的关键要素。如果代码正经历审核，审核人告诉你有些东西不够清晰，不要与他们争辩，如果读者认为不够清晰，那就是不够清晰。尝试理解为什么他们会感到困惑，看看自己能否让代码变得更加清晰，比如通过更好的注释或更好的代码，而不是与他们争论。

### 13.9 13.5 节问题的答案

为了使用 IndexLookup 类，开发者是否需要知道关于该类的所有信息？

1. IndexLookup 类向索引服务器发送的消息格式。不需要：这是实现的细节，应该被隐藏在类中。
2. 用于确定特定对象是否满足期望范围的对比方法（对比方法是使用整型、浮点型、数字、还是字符串？）。需要：使用该类时，需要知晓该信息。
3. 服务器存储索引的数据结构。不需要：该信息应该封装在服务器内，甚至 IndexLookup 的实现都不应该知道该信息。
4. IndexLookup 是否是通过并发的方式，向多个不同的服务器发起请求。可能需要：如果 IndexLookup 使用了特殊技术来优化性能，那文档应该提供一些关于该技术的高层信息，因为用户可能会关心性能。
5. 处理服务器崩溃的机制。不需要：RAMCloud 会自动从服务器崩溃中恢复，因此无需在 IndexLookup 的接口文档中提及崩溃。如果崩溃会反映到应用程序中，那接口文档应描述这些崩溃的表现形式（但不应涉及崩溃恢复的工作细节）。