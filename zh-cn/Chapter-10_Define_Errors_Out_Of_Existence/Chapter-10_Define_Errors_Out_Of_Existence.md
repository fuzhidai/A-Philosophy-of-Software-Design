...
减少必须处理异常的位置数量，第二种的方法是异常屏蔽。通过这种方式，一种异常情况被检测到，并在系统的底层中被处理掉，此时软件的上层将不再会感知到这种异常情况。异常屏蔽在分布式系统中非常常见，例如，在一个网络传输协议中，例如 TCP，网络包可能会因为多种原因而丢失，比如受到损坏或者出现拥塞。但 TCP 在其实现中，会重新发送丢失的网络包，因此所有的数据最终都会抵达，客户端也不会意识到丢包的情况。

一个更加具有争议的屏蔽例子，出现在 NFS 网络文件系统中。如果一个 NFS 网络服务崩溃，或者因为某种原因而无法响应，客户端将会重新发送它们的请求，一遍又一遍，直到问题最终被解决。文件系统的底层代码，不会对调用方的应用抛出任何异常。进行中的操作只是会一直挂在那里，直到操作能够成功完成。如果挂起持续较长的一段时间，NFS 客户端会在用户的控制台上打印如下格式的信息：“NFS 服务 xyzzy 没有响应，持续重试中”。

NFS 的用户经常会抱怨，他们的应用在等待 NFS 服务恢复执行正常操作时被挂起。许多人建议 NFS 应该通过异常来阻断操作，而不是将其挂起。可是反馈异常将会让事情变得更加糟糕，而非更好。如果一个应用失去了其对文件的访问权限，它将无法进行任何操作。一个可能的方案是，由应用发起对文件操作的重试，但这仍会挂起应用程序，保证仅在一个 NFS 层进行重试，而非在每个应用调用的所有文件系统发起重试（一个编译器则不需要担心这个问题）。另一个方案是应用终止，并将错误返回给它们的调用者。而调用方也不太可能知道如何进行处理，所以他们也会选择终止，最终导致整个用户的工作环境崩溃。当服务下线时，用户仍然无法完成任何工作，他们只能在文件服务恢复后，重启他们所有的应用。

因此，对于 NFS 来说，最好的方式就是屏蔽异常，并挂起应用。通过这种方式，应用无需编写任何代码来处理服务问题，他们能够在服务恢复后，无缝的完成恢复。如果用户厌烦了等待，他们总是可以手动的终止应用。

异常屏蔽并非在所有的场景中都适用，但它在它能够生效的场景下是非常高效的工具。它使得类变得更加深层，因为它减少了类的接口信息（用户需要感知的异常更少），并以屏蔽异常的代码形式添加功能。异常屏蔽是复杂度下沉的一个例子。